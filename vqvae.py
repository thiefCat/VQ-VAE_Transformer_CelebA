...
def build_config_from_args(is_jupyter=False):
    ...
    parser.add_argument('--num_embeddings', type=int, default=256, help="length of the codebook")
    parser.add_argument('--num_res_layers', type=int, default=2, help="number of stacked residual blocks")
    parser.add_argument('--alpha', type=float, default=1.5, help="The hyperparameter controlling the balance between rec loss and codebook loss")
    ...
    parser.add_argument('--out_dir', type=str, default="VQ-VAE-L/results", help="Flag for output logging")
    if is_jupyter:
...
def load_dataset(train_batch_size=256):
    train_loader, test_loader = celeba64.get_celeba_data(train_batch_size), None
    return train_loader, test_loader


class Experiment:
    def __init__(self, train_data, model: nn.Module,
...
if __name__ == '__main__':
    ...
    print(device)
    train_loader, test_loader = load_dataset(train_batch_size=256)  # the training data is in range [-1, 1]
    in_dim = 3
    ...
    experiment = Experiment(train_loader, model, lr=0.0025, num_epochs=12, config=config)
